...
- 💬 **[G6] Real login + cross-device profile** — BUILT + locally verified 2026-07-11, commit `7abb3eb` LOCAL (unpushed) in storyreader-gunner. Passkeys primary + email-code fallback + passkey list/add/remove; profile root cause = AccountSection buried last in Settings → now FIRST as a raised card; magic-link cross-browsing-context issue documented. Migration `worker/migrations/0002_passkey_credentials.sql` (additive) NOT applied to remote. **Ship = user says "apply the passkey migrations"** → apply gunner+holdfast migrations, push both commits, auto-deploy.
...
- 💬 **[H4] Real login + cross-device profile** — BUILT + locally verified 2026-07-11, commit `d24d6e7` LOCAL (unpushed) in storyreader-holdfast. Profile root cause = auth redirects to `/?auth=ok` that nothing read + magic-link cross-context → now redirects to Settings with a visible banner + real profile card. Migration `worker/migrations/0002_passkeys.sql` (additive) NOT applied to remote. Ships with G6 on the user's word.
