...
- 🔄 **[copy] Em-dash purge, both reader apps** — holdfast ✅ DONE (`35bdb1a`: 36 fixes across 11 files incl. sw.ts push text + CHANGELOG/ROADMAP rendered into About; zero unjustified survivors; typecheck + both builds green). Gunner sweep 🔄 still running. (Scene-break `— · —` glyph in BlockRenderer is decorative/aria-hidden — exempt.)
