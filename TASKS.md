...
- 🔵 **[VIDEO] Gunner video / trailers — FUTURE (post image+voice run)**: set aside only because MAI has no video model. Two paths. **(A)** stills + narration — Ken-Burns pan/zoom over the generated MAI illustrations + MAI-Voice-2 narration, assembled with ffmpeg; no new model, ~free. **(B) True AI video = OpenAI Sora via Azure AI Foundry** — a separate model deployment on the foundry (`aif-studioai-prod-eus-01`). Must **verify Sora is available in the East US resource** (Sora's region rollout is limited) before promising it. It is the genuine "credit sink" — meaningfully pricier per clip. Owner to choose A vs B; video was always on the plan, just parked pending this.
...
- ✅ **[A2/G5] AI voice for Listen (MAI-Voice-2)** — DONE 2026-07-12. All 3 listen voices (Harper, Isla `en-AU-Isla:MAI-Voice-2`, Ethan excited) rendered for all 42 stories, live on R2, picker fully functional. Fixed a real bug along the way: `tools/tts.mjs` had a stale hardcoded 3.1s-per-block throttle assuming the old free F0 tier; replaced with bounded concurrency (`storyreader-gunner` `295ce7a`) — ~12x faster, no more F0/S0 confusion going forward.
- ✅ **[G11] Full illustrated art — DONE 2026-07-12.** All 42 stories regenerated via MAI-Image-2.5: 40/42 covers (stories 24 and 41 blocked by the image API's content-safety filter even after two rounds of prompt rewording — deprioritized, app falls back to the brand icon for those two), all 42 stories have new scene art. Found + fixed a silent pipeline bug: every story's `coverImage` frontmatter pointed at a filename (`story-NN.png`) that the generator never actually writes (`gunnerthelab.github.io` `64f0c4b`) — covers were never reaching the app until this was caught.
- ✅ **[G13] Immersive illustrated theme — DONE 2026-07-12.** Now Playing, Library, and Home now show a blurred version of the story's cover art as a background (Now Playing: full-bleed behind the player; Library/Home: a hero band behind just the header), with a `--bg`-derived scrim so contrast holds automatically in both light and dark themes (WCAG AA verified for worst-case pixel extremes, ~10-11:1). Reader's read-along text screen intentionally left on its plain surface — dense text + background art was judged a legibility risk. `storyreader-gunner` `d56b3cd`. Shipped as **v0.3.0** (`a56e9f2`), see `docs/CHANGELOG.md`.
...
- 2026-07-12: **Gunner full MAI-Image + MAI-Voice-2 run complete.** All 42 stories have new scene art, 40/42 have covers (24 and 41 blocked by the image API's content-safety filter), all 3 listen voices fully rendered for all 42 stories, cover-publish pipeline bug fixed, new illustrated-background theme shipped (v0.3.0). Holdfast side untouched by this run — `[A2/H3]` (Holdfast listen voices) still open. Canon QA spot-check (13 images across eras) found no hard violations; flagged story-24's missing cover (already known) and two night-scene images whose sepia treatment reads slightly light-toned (likely stylistic, not a color bug) for a human glance. Marketing/branding art (logos, OG image, YouTube banner, etc.) not yet generated — needs new tooling beyond the existing per-story `mai-image.mjs`, in progress.
